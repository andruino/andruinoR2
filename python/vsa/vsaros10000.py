...
class vsa(object):
	...
	def __init__(self,NDIMS):
		...
		if  os.path.exists('sensors.dat') and os.path.exists('actuators.dat') and os.path.exists('controller.dat'):
			No_ficheros=False
		else:
			No_ficheros=True
...
def callback(msg):
	...
	if len(list)==13:
		...
		values0[SENRANGESIGN + int(int(list[0])/10)]=1
		values0[SENRANGESIGN+10+int(int(list[2])/10)]=1
		values1[SENRANGESIGN+int(int(list[3])/10)]=1
		values1[SENRANGESIGN+10+int(int(list[5])/10)]=1
		...
		if ((LDRmediapost+MIN_MEJORA) < LDRmedia): 
			...
			#Desaprende
			if LDRmediapost > (LDRmedia+MIN_MEJORA):			
